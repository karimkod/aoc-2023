...
public static class DayOne
{
    public static void Run()
    {
        ...
        var value = CalibrationValueDetector.Detect(input.Split(Environment.NewLine));

        Console.WriteLine(value);
    }
}

public static class CalibrationValueDetector
{
    public static int Detect(params string[] input)
    {
        int result = 0;
        foreach (string s in input)
        {
            result += DetectSingleLine(s);
        }

        return result;
    }

    private static int DetectSingleLine(string s)
    {
        char? firstInt = null;
        int i = 0;
        while (i < s.Length)
        {
            ...
            if (DetectSpelledNumber(out firstInt, i, s))
                break;
        ...
        while (j <= s.Length)
        {
            if (int.TryParse(s[^j].ToString(), out _))
            {
                lastInt = s[^j];
                break;
            }
            
            if (DetectSpelledNumber(out lastInt, s.Length - j , s))
                break;

            j++;
        }

        if (lastInt is null && firstInt is null)
        {
            lastInt = '0';
            firstInt = '0';
        }
    ...
    private static bool DetectSpelledNumber(out char? number, int startingIndex, string s)
    {
        string[] numbers = ["one", "two", "three", "four", "five", "six", "seven", "eight", "nine"];
        
        number = null;
        for (int i = 2; i <= 5; i ++ )
            if (startingIndex + i < s.Length)
            {
                var indexOfNumber = Array.IndexOf(numbers, s[startingIndex..(startingIndex + i + 1)]);
                if (indexOfNumber != -1) 
                { 
                    number = (indexOfNumber + 1).ToString()[0];
                    return true;
                }
            }
        return false;
